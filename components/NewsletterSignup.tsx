...
const NewsletterSignup: React.FC = () => {
  ...
  const handleSubmit = async (event: React.FormEvent<HTMLFormElement>) => {
    ...
    if (error) {
      ...
      if (error.code === '23505') { // Unique constraint violation
        ...
        setMessage(t('errorOccurred') + '. ' + t('tryAgainLater'));
      }
  ...
  return (
    <div className="bg-white/50 dark:bg-slate-800/50 backdrop-blur-sm border border-slate-200/80 dark:border-slate-700/50 rounded-2xl p-8 max-w-2xl mx-auto text-center">
      <h2 className="text-2xl sm:text-3xl font-bold text-slate-900 dark:text-slate-100">{t('stayUpToDate')}</h2>
        ...
        <form onSubmit={handleSubmit} className="mt-8 flex flex-col sm:flex-row gap-4 max-w-md mx-auto">
          <div className="relative flex-grow">
            ...
            <input
              ...
              placeholder={isSupabaseConfigured ? t('emailAddress') : t('signUpDisabled')}
              required
          ...
          >
            {status === 'submitting' ? t('saving') : t('signUp')}
          </button>
        </form>
      ) : null}
